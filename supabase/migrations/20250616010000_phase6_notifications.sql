-- Phase 6: notification worker schema — skipped status, reminders toggle, claim batch, owner enqueue
...
ALTER TYPE public.notification_template ADD VALUE IF NOT EXISTS 'owner_new_booking';

-- ---------------------------------------------------------------------------
-- shops.reminders_enabled
-- ---------------------------------------------------------------------------

ALTER TABLE public.shops
  ADD COLUMN IF NOT EXISTS reminders_enabled boolean NOT NULL DEFAULT true;

COMMENT ON COLUMN public.shops.reminders_enabled IS
  'Owner toggle: when false, worker skips pending reminder_24h rows at dispatch (future reminders only).';

-- ---------------------------------------------------------------------------
-- notification_outbox.claimed_at — crash-safe claim without a sending status
-- ---------------------------------------------------------------------------
--
-- State machine (status + claim metadata):
--   pending  → sent      (successful send; provider id stored in payload)
--   pending  → skipped   (category disabled or appointment no longer relevant)
--   pending  → failed    (transient error; scheduled_for bumped for retry)
--   failed   → sent | skipped | failed | dead  (retry path when scheduled_for due)
--   pending/failed → dead (attempts ≥ 5 after a failed send)
--
-- Claim: increments attempts, sets claimed_at = now(). Row stays pending/failed — no sending state.
-- Crash recovery: pending/failed rows with stale claimed_at (> 5 min) are reclaimable.
-- Recipient resolution: payload.to holds the email for every template (no schema change).

ALTER TABLE public.notification_outbox
  ADD COLUMN IF NOT EXISTS claimed_at timestamptz;

COMMENT ON TABLE public.notification_outbox IS
  'Email outbox queue. Worker claims via claim_outbox_batch(); booking RPCs insert rows.';

-- ---------------------------------------------------------------------------
-- enqueue_booking_outbox_rows — customer + owner notifications after book
-- ---------------------------------------------------------------------------

CREATE OR REPLACE FUNCTION public.enqueue_booking_outbox_rows(
  p_shop_id uuid,
...
BEGIN
  IF p_customer_email IS NOT NULL AND btrim(p_customer_email) <> '' THEN
    ...
    VALUES (
      ...
      jsonb_build_object('to', p_customer_email, 'appointment_id', p_appointment_id),
      pg_catalog.now()
    );

    v_reminder_at := p_starts_at - interval '24 hours';
  ...
  -- Owner always has exactly one membership; auth email is the recipient (payload.to).
  SELECT u.email::text
  INTO v_owner_email
  ...
  IF v_owner_email IS NOT NULL AND btrim(v_owner_email) <> '' THEN
    ...
    VALUES (
      ...
      p_appointment_id,
      'email'::public.notification_channel,
      'owner_new_booking'::public.notification_template,
      jsonb_build_object('to', v_owner_email, 'appointment_id', p_appointment_id),
      pg_catalog.now()
    );
  END IF;
END;
...
COMMENT ON FUNCTION public.enqueue_booking_outbox_rows(uuid, uuid, timestamptz, text) IS
  'Inserts booking_confirmed, optional reminder_24h, and owner_new_booking outbox rows.';

REVOKE ALL ON FUNCTION public.enqueue_booking_outbox_rows(uuid, uuid, timestamptz, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.enqueue_booking_outbox_rows(uuid, uuid, timestamptz, text) TO service_role;

-- ---------------------------------------------------------------------------
-- book_appointment — delegate outbox inserts to helper (adds owner_new_booking)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- claim_outbox_batch — service-role worker claim (SKIP LOCKED, crash reclaim)
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  WITH picked AS (
    ...
    WHERE no.status IN (
      ...
      AND no.scheduled_for <= pg_catalog.now()
      AND (
        no.claimed_at IS NULL
        OR no.claimed_at < pg_catalog.now() - interval '5 minutes'
      )
  ...
  SET
    ...
    updated_at = pg_catalog.now()
  FROM picked
  WHERE no.id = picked.id
  RETURNING no.*;
END;
$$;

COMMENT ON FUNCTION public.claim_outbox_batch(integer) IS
  'Atomically claims due outbox rows for the notification worker. Service role only.';
