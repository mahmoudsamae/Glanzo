...
import { forgeHeroEnter } from "@/lib/minisite/forge-motion";
...
import { minisiteImageUrl, shopMediaPublicUrl } from "../../lib/media-url";
...
export function ForgeHeroSection({ data }: ForgeHeroSectionProps) {
  ...
  const videoPath = content.show?.cover !== false ? content.cover_video_path?.trim() : undefined;
  const videoUrl = videoPath ? shopMediaPublicUrl(videoPath) : null;
  const posterUrl = heroImage ? minisiteImageUrl(heroImage) : undefined;
          ...
          <div className="ms-forge-hero-image-wrap">
            {videoUrl ? (
              <video
                ...
                poster={posterUrl}
                aria-hidden
                data-forge-hero-video
              >
                <source src={videoUrl} type={heroVideoMime(videoPath ?? "")} />
              </video>
            ) : (
              <NicolesPhoto
                path={heroImage}
