...
type SignatureVisitPanelProps = {
  data: ShopPublicData;
};

export function SignatureVisitPanel({ data }: SignatureVisitPanelProps) {
  ...
  return (
    <footer
      ...
      className="ms-signature-footer ms-signature-section ms-cinema-section"
    >
      <div className="mx-auto grid max-w-5xl gap-[var(--space-8)] px-[var(--space-4)] py-[var(--space-12)] min-[768px]:grid-cols-3">
        {showLocation && address ? (
          <article>
            <h2 className="font-display text-xl text-[color:var(--ms-signature-cream)]">Termin vereinbaren</h2>
            {mapsHref ? (
              <a
                href={mapsHref}
                target="_blank"
                rel="noopener noreferrer"
                className="mt-[var(--space-3)] block text-md leading-relaxed text-[color:color-mix(in_oklch,var(--ms-signature-cream)_82%,transparent)] underline-offset-4 hover:underline"
              >
                {address}
              </a>
            ) : (
              <p className="mt-[var(--space-3)] text-md leading-relaxed text-[color:color-mix(in_oklch,var(--ms-signature-cream)_82%,transparent)]">
                {address}
              </p>
            )}
          </article>
        ...
        {showHours ? (
          <article>
            <h2 className="font-display text-xl text-[color:var(--ms-signature-cream)]">Öffnungszeiten</h2>
            <dl className="mt-[var(--space-3)] space-y-[var(--space-2)]">
              {hours.map((line) => (
                <div key={line.label} className="flex justify-between gap-[var(--space-4)] text-sm">
                  <dt className="text-[color:color-mix(in_oklch,var(--ms-signature-cream)_70%,transparent)]">
                    {line.label}
                  </dt>
                  <dd className="tabular-nums text-[color:var(--ms-signature-cream)]">{line.value}</dd>
                </div>
              ))}
            </dl>
          </article>
        ...
        {showSocial ? (
          <article className="min-[768px]:text-right">
            <h2 className="font-display text-xl text-[color:var(--ms-signature-cream)]">Folgen</h2>
      ...
      <div className="ms-signature-footer-bar px-[var(--space-4)] py-[var(--space-3)] text-center text-xs text-[color:var(--ms-signature-teal)]">
