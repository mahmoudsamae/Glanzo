...
export async function getShopBySlug(
  ...
  const { data, error } = await supabase
    .from("shops")
    .select(
      "id, slug, name, status, timezone, currency, opening_hours, booking_lead_time_min, cancellation_window_min, slot_granularity_min, booking_auto_assign_barber, reminders_enabled, allowed_minisite_templates, created_at, updated_at",
    )
    .eq("slug", slug)
    .maybeSingle();
